...
setwd("F:\\杂优中心\\杂种优势表达模式\\DESeq2\\stringtieNonovel\\杂优模式\\5P")

d1 <- read.csv("../../DESeq2/readscount大于1过滤/5P_R900-vs-Y58S_diff.csv",header = T)
d2 <- read.csv("../../DESeq2/readscount大于1过滤/5P_R900-vs-Y900_diff.csv",header = T)
d3 <- read.csv("../../DESeq2/readscount大于1过滤/5P_Y58S-vs-Y900_diff.csv",header = T)
...
all_res
write.table(all_res,"tisse_5P_heterosis_pattern_stat.txt",col.names = T,row.names = F,sep = "\t",quote = F)
